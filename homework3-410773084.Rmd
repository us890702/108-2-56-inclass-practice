...
as.factor(badEmployers$law) -> badEmployers$law
...
mostViolatedArticle <- table_countsByArticle[table_countsByArticle == 507]
...
```{r ans14}
pick_lawContent <- table(badEmployers$lawContent)
max(table(badEmployers$lawContent))
pick_lawContent[pick_lawContent == 507] -> mostViolatedArticle_content
names(mostViolatedArticle_content) -> mostViolatedArticle_content
...
table(month(badEmployers$dt)) == 296 ->pick_popularSentenceMonth
table(month(badEmployers$dt))[pick_popularSentenceMonth] -> popularSentenceMonth
...
levels(table$國文) <- levels(table$國文)[c(1,2,11,3,4,5,6,7,8,9,10,12,13,14,15,16)]
...
```{r ans22}
pickntpu <- str_detect(students10$志願,"北大")
ntpu_namelist <- students10$姓名[pickntpu]
ntpu_namelist
```
...
logical儲存的本質是甚麼?
```{r ans23}
pickntpueco <- students10$志願 == "北大經濟"
students10$國文[pickntpueco] >= 12 
students10$英文[pickntpueco]+students10$數學[pickntpueco] >= 26 -> pass2
students10$姓名[pickntpueco][pass2] -> ntpu_econtpu_eco
ntpu_econtpu_eco
```
...
```{r ans24}
namelist
ntpu_econtpu_eco %in% namelist -> ntpu_congrats
paste0("恭喜",ntpu_econtpu_eco[ntpu_congrats],"同學錄取國立臺北大學經濟系") -> congrats
congrats
```
