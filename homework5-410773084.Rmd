...
sort(as.integer(levels(studentHeights$學年度))) -> levels(studentHeights$學年度)
studentHeights$年齡 <- factor(studentHeights$年齡, ordered = T)
sort(as.integer(levels(studentHeights$年齡))) -> levels(studentHeights$年齡)
...
```{r ans12}
print(studentHeights$年齡層)
studentHeights$年齡層 <- factor(studentHeights$年齡層, ordered = T)
levels(studentHeights$年齡層) <- levels(studentHeights$年齡層)[c(2,3,1)]
...
averageHeightByGroup <- c(mean(studentHeights$平均身高[pick_68&pick]),mean(studentHeights$平均身高[pick_912&pick]),mean(studentHeights$平均身高[pick_1315&pick]))
...
```{r ans14}
pickman <- studentHeights$類別 == "男"
pickwoman <- studentHeights$類別 == "女"
heightDifferenceByGroup <- c( mean(studentHeights$平均身高[pick_68&pickman]) -mean(studentHeights$平均身高[pick_68&pickwoman]),mean(studentHeights$平均身高[pick_912&pickman]) -mean(studentHeights$平均身高[pick_912&pickwoman]),mean(studentHeights$平均身高[pick_1315&pickman]) -mean(studentHeights$平均身高[pick_1315&pickwoman])       )
...
```{r ans23}
bSchoolRegex <- "(金融與?合?作?經?營?學?系|休閒?與?運動?管?理?學?系)"
bSchoolRegex
```
...
```{r ans24}
levels(differentMajors) <- c("不動系","公行系","公行系","公行系","金融系","金融系","金融系","經濟系","經濟系","社工系","社工系","社會系" ,"社會系" ,"休運系","休運系")
...
levels(school) -> level1
b_regex <- "休運系|金融系"
e_regex <- "經濟系|社會系|社工系"
d_regex <- "公行系|不動系"
level1 <- str_replace(level1, b_regex,"商學院")
level1 <- str_replace(level1, e_regex,"社科院")
newLevels <- str_replace(level1, d_regex,"公共事務學院")
levels(school) <- newLevels
class(school)
levels(school)
school
```
